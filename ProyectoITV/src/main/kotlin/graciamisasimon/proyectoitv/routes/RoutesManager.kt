package graciamisasimon.proyectoitv.routes

import graciamisasimon.proyectoitv.controllers.MainController
...
object RoutesManager {
    ...
    enum class View(val fxml: String) {
        ...
        EDICION("views/revision-view.fxml"),
    }
    ...
    fun initAcercaDeStage() {
        ...
        val stage = Stage()
        stage.title = "Acerca De Proyecto ITV"
        stage.scene = scene
    ...
    fun initDetalle() {
        ...
        val fxmlLoader = FXMLLoader(getResource(View.EDICION.fxml))
        val parentRoot = fxmlLoader.load<Pane>()
        val scene = Scene(parentRoot, 350.0, 400.0)
        val stage = Stage()
        stage.title = "Edicion Cita"
        stage.scene = scene
        stage.initOwner(mainStage)
        stage.initModality(Modality.WINDOW_MODAL)
        stage.isResizable = false
